...
BROSX_HOME=$BROSX_ROOT/opt/buildroot
...
BROSX_BINUTILS_HOME=/usr/local/Cellar/binutils/2.29.1/bin
...
#BROSX_LIBTOOL_HOME=/usr/local/Cellar/libtool/2.4.6_1/bin
...
BROSX_SSH_CMD=/usr/ssh/less
BROSX_VI_CMD=/usr/vi/less
...
PS1="\[\e[0;32m\]buildroot\[\e[0m\]\[\e[1;32m\]@\[\e[0;32m\]\h\[\e[1;32m\]:\[\e[0;37m\]\w\[\e[0m\]$ " 
...
for var in ${!BROSX_*}; do 
	name=`echo $var | sed "s/BROSX_\([^_]*\)_\([^_]*\)/\1/"`
...
function linkWithoutGnuPrefix() { 
	...
	for gCmd in `ls`; do  
		cmd=${gCmd#g}; 
		if [ ! -f $dstDir/$cmd ]; then 
			echo "- installing $envVar: link '$srcDir/$gCmd' -> '$dstDir/$cmd'";
			ln -s $srcDir/$gCmd $dstDir/$cmd;
		fi 
	done;  
	popd; 
}

BROSX_TOOLS_PATH="$BROSX_ROOT/opt/tools/bin"
BROSX_PATH=$BROSX_TOOLS_PATH

# link simple commands
for cmd in $cmds; do
	var=BROSX_${cmd}_CMD
	eval "cmdPath=\${$var}"
	cmdLink=$BROSX_TOOLS_PATH/${cmd,,}
	if [ ! -f $cmdPath ]; then 		
		echo "- installing $var: link  '$cmdPath' -> '$cmdLink'";
...
export TERM LS_COLORS PS1 M4 PATH ${!BROSX_*}
